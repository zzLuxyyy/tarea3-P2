...
ENTREGAR = fecha libro socio prestamo lseSocios ldePrestamos abbLibros reserva colaReservas agGeneros conjuntoGeneros biblioteca
...
# casos de prueba
IDS_PUBLICOS = reserva1-crear-eliminar \
...
t-%:$(INDIR)/%.in $(OUTDIR)/%.out
	@mkdir -p $(SALIDADIR)
	@timeout 2 valgrind -q --leak-check=full ./$(EJECUTABLE) < $< > $(SALIDADIR)/$*.sal 2>&1 || true
	@diff $(OUTDIR)/$*.out $(SALIDADIR)/$*.sal || true
	@if diff $(OUTDIR)/$*.out $(SALIDADIR)/$*.sal > /dev/null; then \
		echo ---- Bien ----; \
	else \
		echo ---- Mal ----; \
	fi
